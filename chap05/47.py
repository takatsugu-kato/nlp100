...
with open('./47.txt', 'w', encoding='utf-8') as out_file:
    for i, sentence in enumerate(morpheme_neko):
        for j in range(len(sentence)):
            if len(sentence[j].morphs) < 2:#サ変接続だけしかないチャンクの場合はmorphs[1]がout of indexになるので
                continue
            if (sentence[j].morphs[0].pos1 == "サ変接続" and sentence[j].morphs[1].surface == "を" and sentence[j+1].morphs[0].pos == "動詞"):
                kakus = []
                kous = []
                kou_and_kakus = []
                for src_id in sentence[j+1].srcs:
                    if src_id == j:
                        continue
                ...
                if (kou_and_kakus):
                    ...
                    out_file.writelines(sentence[j].get_chunk_surface()+sentence[j+1].get_chunk_base() + "\t" + " ".join(kakus) + "\t" + " ".join(kous) + "\n")
                    print (sentence[j].get_chunk_surface()+sentence[j+1].get_chunk_base() + "\t" + " ".join(kakus) + "\t" + " ".join(kous))
